...
const img1 = "https://posterandframe.com/cdn/shop/files/Tim-David_Red-eyed-tree-frog_40-x-50_PLD_7950_5713713580237_1024x1024.jpg?v=1698237068"
const img2 = "https://images.wall-art.de/format:webp/q:92/rs:fit:1560:/_img/out/pictures/master/product/1/glasbild-rund-ga16161-gruener-frosch-30x30-final.jpg"
    return(
        <>
        <input value={input} onChange={checkInput} type="text" />
        <h1>Count :{count}</h1>
        <button  className="btn" onClick={increment}>+1</button><button  className="btn" onClick={decrement}>-1</button>
       <button onClick={addRandomColor}></button>
       {array.map((colors, index)=> {
        return  <div key={index} className="colors__div" style={{backgroundColor:colors}}>{colors}</div>
        
       })}

       {object.array.map((data)=> {
        return <div className="object" style={{backgroundColor:data, height:object.number + "px"}}>{object.value}
        <img className="object__img" src={object.boolean ? img1 : img2} alt="frog" />
